...
function call(){
	...
	selectedPatient = "Sally";
	var numPatients = 4; // 1. get the patients' icons from the file
	...
	var nameBox = mainBox.append("rect")
		...
		.on("click", function(d) { 
						d3.selectAll(".dropdown-toggle").style({'display':'block'}); } );
	...
	var patientText = mainBox.append("text")
		.attr("x", 175).attr("y", 405).text("").attr("font-size", 30)
		.attr("class","large-title");
	...
	dropdown
		.style({'display':'none'}) // dropdown should start out hidden
		.attr("class","dropdown-toggle")
		.attr("x", 150).attr("y", 370).attr("rx", 5).attr("ry", 5)
		.attr("height", (60*numPatients) + 10*((numPatients>0)?(numPatients-1):0)).attr("width", width-100)
		.attr("fill", "white").attr("stroke", "black").attr("stroke-width", 0.2);
	// 3. fill in the contents of the menu
	for(i = 0; i < 1; i++) { //>
		var patientName = "sallyuser1990"; // gotta populate this somehow
		mainBox.append("rect")
			.attr("display","none")
			.attr("class","dropdown-toggle")
			.attr("id",patientName)
			.attr("x", 151)
			.attr("y", 370+(50*i + (1*( i + 1) + 1)))
			.attr("rx", 5).attr("ry", 5)
			.attr("height",50).attr("width",width-102)
    		.attr("fill", "white").attr("stroke", "black").attr("stroke-width", 0.2)
    		.on("mouseover", function(d) { mouseoverChange(d3.select(this)) })
    		.on("mouseout", function(d) { mouseoutChange(d3.select(this)) })
			.on("click",function(d) { 
			  console.log("Hi, Sally!"); 
			  d3.selectAll(".dropdown-toggle").style({'display':'none'});
			  selectedPatient = d3.select('#sallyuser1990').attr("id");
			  d3.select("#name").text("sallyuser1990"); });
		mainBox.append("text")
			.attr("class","text dropdown-toggle")
			.style({'display':'none'})
			.attr("x", 170).attr("y", 405 + (50*i + (1*(i+1) + 1)) ).text(patientName).attr("font-size", 25);
	}
